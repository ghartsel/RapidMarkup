...
Included files within a section are located in the /module/<section>/partials directory.

Globally included files are located in the /modules/&#95;includes/partials directory.
...
Nested list items are prefaced with an asterisk (`*`) and set off by blank lines:
...
Use the `[.procedure]` macro to define an ordered list and number each step using a dot (`.`).

----
[.procedure]
. Log in to the dashboard.

. Choose uitext:Backups[] in the left navigation panel to view your
databases and the database backup properties.

. In the row of the database you want to change backup properties for,
click the uitext:ACTIONS[] icon on the far right.
----

Which is rendered as the following with numeric icons:

> . Log in to the dashboard.
> . Choose **Backups** in the left navigation panel to view your databases and the database backup properties.
> . In the row of the database you want to change backup properties for, click the **ACTIONS** icon on the far right.

An ordered list should not have nested steps. See xref:./style_guide.md#ordered-lists[Ordered lists].
...
----
Database::
  A database can have Collections, Documents, User-defined functions, security
  elements such as Keys, Tokens, Credentials, Access Providers, and child
  databases.
----
...
This displays `Set` with a link to the set description in the FQL language reference Type document.
