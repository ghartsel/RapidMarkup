...
| `alternate`
| (optional) Text to display if image fails to load.
...
`zoom` = (optional) Enable zoom on clicking on image.
|===
...
A badge is defined by appending the badge specification, in parentheses: `= `list-endpoints` (deprecated)`
